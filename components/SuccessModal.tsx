...
const SuccessModal: React.FC<SuccessModalProps> = ({ isOpen, onClose, data }) => {
  ...
  return (
    <div className="fixed inset-0 z-[2000] flex items-center justify-center p-3 sm:p-6 md:p-8 bg-navy-950/98 backdrop-blur-3xl animate-in fade-in duration-500 overflow-y-auto">
      ...
      <div className="relative w-full max-w-5xl bg-navy-900 border border-white/10 rounded-[2rem] md:rounded-[4rem] overflow-hidden shadow-[0_50px_100px_-20px_rgba(0,0,0,0.8)] animate-modal-elastic flex flex-col md:flex-row my-auto max-h-[95vh] md:max-h-none overflow-y-auto custom-scrollbar">
        ...
        <div className="relative w-full md:w-2/5 bg-navy-950 p-6 sm:p-8 md:p-12 flex flex-col justify-between overflow-hidden border-b md:border-b-0 md:border-r border-white/5">
          ...
          <div className="relative z-10 mb-8 md:mb-0">
            ...
            <h2 className="text-3xl sm:text-4xl md:text-6xl font-black uppercase italic tracking-tighter leading-[0.85] mb-4">
              READY TO <br />
          ...
          <div className="relative z-10 space-y-3">
            ...
            <div className="p-4 sm:p-5 md:p-6 bg-white/5 rounded-2xl md:rounded-3xl border border-white/10 backdrop-blur-md">
              <p className={`text-xs md:text-base font-medium leading-relaxed italic transition-all duration-1000 ${isLoadingAdvice ? 'text-white/10 blur-sm' : 'text-white/90'}`}>
        ...
        <div className="w-full md:w-3/5 bg-navy-900 p-6 sm:p-8 md:p-12 flex flex-col">
          <div className="flex justify-between items-start mb-8 md:mb-12">
            <div className="space-y-1">
              <h3 className="text-lg md:text-xl font-black uppercase tracking-tight italic text-white/50">Registration Info</h3>
          ...
          <div className="flex-1 space-y-8 md:space-y-10">
            ...
            <div className="flex items-center gap-4 md:gap-6">
              ...
              <div className="space-y-0.5 min-w-0">
                ...
                <p className="text-xl md:text-2xl font-black uppercase italic tracking-tight truncate">{data.fullName}</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-2 gap-4 md:gap-8 py-6 md:py-8 border-y border-white/5">
              <div className="space-y-1 min-w-0">
                <span className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1.5">
                  <MapPin size={10} className="text-brand-red shrink-0" /> Destinasi
                </span>
                <p className="text-base md:text-xl font-black uppercase italic text-white truncate">{data.mountain}</p>
              </div>
              <div className="space-y-1 min-w-0">
                <span className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-[0.2em] flex items-center gap-1.5">
                  <Calendar size={10} className="text-brand-red shrink-0" /> Tanggal
                </span>
                <p className="text-base md:text-xl font-black italic text-white truncate tabular-nums">
              ...
              <div className="space-y-1">
                ...
                <p className="text-[10px] md:text-sm font-black uppercase text-brand-red truncate">{data.packageCategory}</p>
              </div>
              <div className="space-y-1">
                <span className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-[0.2em]">Paket</span>
                <p className="text-[10px] md:text-sm font-black uppercase text-brand-red truncate">{data.tripPackage}</p>
              </div>
            </div>
          </div>
          ...
          <div className="mt-8 md:mt-12 space-y-4">
            <button 
              onClick={onClose}
              className="w-full py-5 md:py-6 bg-brand-red text-white font-black uppercase tracking-[0.3em] md:tracking-[0.4em] rounded-[1.5rem] md:rounded-[2rem] shadow-glow-red hover:scale-[1.02] active:scale-95 transition-all flex items-center justify-center gap-3 italic text-[11px] md:text-sm"
            >
              <Download size={18} />
            ...
            <div className="flex items-center justify-center gap-2">
              ...
              <span className="text-[8px] md:text-[9px] font-black text-slate-500 uppercase tracking-widest text-center px-4">Cloud Sync Active • Data Terdaftar Secara Resmi</span>
            </div>
          </div>
        </div>
